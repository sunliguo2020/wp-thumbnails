...
function ta_image_resize( $file, $max_w, $max_h, $crop=false, $suffix=null, $dest_path=null, $jpeg_quality=90, $filename=null) {
	$image = ta_load_image( $file );
	if ($image == false) return false;
	
	//getimagesize()函数返回原图像属性, 赋值给一组变量
	list($orig_w, $orig_h, $orig_type) = getimagesize( $file );
	//预处理尺寸，为生成缩略图作准备
	$dims = ta_image_resize_dimensions($orig_w, $orig_h, $max_w, $max_h, $crop);
	if (!$dims)
 ...
 *
 */
function ta_load_image( $file ) {//直接由url获得
	if ( ! file_exists( $file ) ) {
...
function ta_image_resize_dimensions($orig_w, $orig_h, $dest_w, $dest_h, $crop=false) {
	...
	if ( $crop ) {
		...
		list( $new_w, $new_h ) = ta_constrain_dimensions( $orig_w, $orig_h, $dest_w, $dest_h );
	}

	// if the resulting image would be larger we don't want to resize it
...
function ta_constrain_dimensions( $current_width, $current_height, $max_width=0, $max_height=0 ) {
